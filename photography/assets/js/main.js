...
var main = (function($) { var _ = {
	...
	initEvents: function() {
		...
		// Click photo to toggle caption visibility.
			_.$viewer.on('click', function(event) {
				if (!$(event.target).closest('.nav-next, .nav-previous, .toggle').length)
					_.$body.toggleClass('caption-hidden');
			});
			...
			_.$thumbnails.children()
				.each(function() {
					...
					// Slide.
						...
						// InkFrame photos need contain so the border isn't cropped.
							if ($thumbnail.data('inkframe'))
								s.$slideImage.addClass('inkframe');
							...
							window.setTimeout(function() {
								...
								// Wait for it to load.
									...
									//window.setTimeout(function() {
										...
										// Apply portrait class for tall images so background-size: contain is used.
											if (this.naturalHeight > this.naturalWidth)
												newSlide.$slideImage.addClass('portrait');
											else
												newSlide.$slideImage.removeClass('portrait');
